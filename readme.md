...
The first dataset is a relational database of inscribed epigraphic objects. It is comprised of 26 tables. These have been exported as tab-delimited files encoded in the UTF-8 character set, and may be found in the subdirectory titled `LatinNowDB tables`. (N.B., one table (latnow_source) has been compressed into a ZIP file to circumvent GitHub's 100 MB file size limit.)
